...
def inicializar_base_datos():
    ...
    if cursor.fetchone()[0] == 0:
        usuarios_iniciales = [
            ...
            ("cocina_gen", "1234", "Chef Carlos Gómez", "Encargado", "Cocina General"),
            ("cursos_cocina", "1234", "Chef Sofía Morales", "Encargado", "Cocina Cursos (Parrilla), Cocina Cursos (Repostería)"),
            ("cocina_salud", "1234", "Nutricionista Ana Ruiz", "Encargado", "Cocina area de Salud"),
            ("clinica_dent", "1234", "Dr. Luis Castillo", "Encargado", "Clinica Dental"),
            ("clinica_med", "1234", "Dra. Elena Vargas", "Encargado", "Clinica Médica"),
            ("farmacia_ong", "1234", "Lic. Mario Soto", "Encargado", "Farmacia"),
            ("liceo_ong", "1234", "Prof. Carmen López", "Encargado", "Liceo"),
            ("belleza_mod", "1234", "Lucía Mendoza", "Encargado", "Módulo Uñas Acrílicas, Módulo Maquillaje"),
            ("costura_mod", "1234", "Rosaura Pineda", "Encargado", "Módulo Costura"),
            ("admin_mod", "1234", "Gerardo Ríos", "Encargado", "Administración")
        ]
        cursor.executemany("INSERT INTO usuarios (username, password, nombre_completo, rol, area) VALUES (?, ?, ?, ?, ?)", usuarios_iniciales)
        conexion.commit()
...
if not st.session_state["autenticado"]:
    ...
    if st.sidebar.button("Ingresar"):
        conexion = sqlite3.connect('inventario_ong.db')
        cursor = conexion.cursor()
        cursor.execute("SELECT username, password, nombre_completo, rol, area FROM usuarios WHERE username = ?", (user_ingresado,))
        user_db = cursor.fetchone()
        conexion.close()
        
        # Validamos estrictamente que el usuario exista y la contraseña coincida
        if user_db and user_db[1] == pass_ingresado:
            ...
            # SE ELIMINÓ EL TIP DE AYUDA PARA EVITAR BRECHAS DE SEGURIDAD
        ...
        with st.form("form_cambiar_pass"):
            ...
            if btn_pass:
                if nueva_pass1 != nueva_pass2:
                    ...
                    conexion = sqlite3.connect('inventario_ong.db')
                    cursor = conexion.cursor()
                    cursor.execute("UPDATE usuarios SET password = ? WHERE username = ?", (nueva_pass1, st.session_state['username']))
                    conexion.commit()
                    conexion.close()
    ...
    if st.session_state['rol'] == "Administrador":
        pestana_ver, pestana_registrar, pestana_tarjetas, pestana_usuarios = st.tabs([
            "📦 Ver y Editar Inventario", "➕ Registrar Objeto", "🖨️ Tarjeta de Responsabilidad", "👥 Gestionar Usuarios"
        ])
    else:
        pestana_ver, pestana_registrar, pestana_tarjetas = st.tabs([
            "📦 Ver y Editar Inventario", "➕ Registrar Objeto", "🖨️ Tarjeta de Responsabilidad"
        ])
    ...
    with pestana_ver:
        ...
        if filtro_elegido != "Todas las Áreas":
            cursor.execute("SELECT * FROM inventario WHERE area = ?", (filtro_elegido,))
        else:
        ...
        if registros:
            ...
            for reg in registros:
                ...
                with st.expander(f"[{codigo_reg if codigo_reg else 'S/C'}] {nombre_reg} - Área: {area_reg}"):
                    op_ver, op_editar, op_eliminar = st.tabs(["👁️ Detalle", "✏️ Editar Registro", "🗑️ Eliminar"])
                        ...
                        with st.form(f"form_editar_{id_reg}"):
                            ...
                            if btn_guardar_cambios:
                                if not nuevo_nombre.strip() or not nueva_desc.strip() or not nuevas_obs.strip() or not nuevo_resp.strip():
                                    st.error("⚠️ Error: Todos los campos marcados son obligatorios.")
                                else:
                                    try:
                                        conexion = sqlite3.connect('inventario_ong.db')
                                        cursor = conexion.cursor()
                                        cursor.execute('''
                                            UPDATE inventario 
                                            SET codigo = ?, cantidad = ?, nombre = ?, marca = ?, descripcion = ?, observaciones = ?, estado = ?, area = ?, responsable = ?
                                            WHERE id = ?
                    ...
                    with op_eliminar:
                        st.warning("⚠️ ¿Estás seguro de eliminar este objeto del inventario?")
                        if st.button("Sí, eliminar definitivamente", key=f"btn_del_{id_reg}"):
                            try:
                                conexion = sqlite3.connect('inventario_ong.db')
                                cursor = conexion.cursor()
                                cursor.execute("DELETE FROM inventario WHERE id = ?", (id_reg,))
                                conexion.commit()
                                conexion.close()
        ...
        with st.form("form_registro"):
            ...
            if btn_enviar:
                if not txt_nombre.strip() or not txt_descripcion.strip() or not txt_observaciones.strip() or not txt_responsable.strip():
                    st.error("⚠️ Faltan campos obligatorios. Por favor llena el Nombre, Descripción, Observaciones y Responsable.")
                else:
                    try:
                        conexion = sqlite3.connect('inventario_ong.db')
                        cursor = conexion.cursor()
    ...
    with pestana_tarjetas:
        st.subheader("🖨️ Generación de Tarjeta de Responsabilidad")
        ...
        if st.button("Generar Tarjeta de Responsabilidad"):
            ...
            conexion = sqlite3.connect('inventario_ong.db')
            cursor = conexion.cursor()
            cursor.execute("SELECT codigo, cantidad, nombre, marca, estado, responsable FROM inventario WHERE area = ?", (area_tarjeta,))
    ...
    if st.session_state['rol'] == "Administrador":
        with pestana_usuarios:
                ...
                if btn_crear_user:
                    if not nuevo_user.strip() or not nuevo_pass.strip() or not nuevo_nombre_completo.strip():
                        ...
                        try:
                            conexion = sqlite3.connect('inventario_ong.db')
                            cursor = conexion.cursor()
                            cursor.execute("INSERT INTO usuarios (username, password, nombre_completo, rol, area) VALUES (?, ?, ?, ?, ?)",
            ...
            for u in lista_usuarios:
                ...
                with col_btn:
                    if uname != "admin":
                        if st.button("🗑️ Eliminar", key=f"del_user_{id_u}"):
                            conexion = sqlite3.connect('inventario_ong.db')
                            cursor = conexion.cursor()
